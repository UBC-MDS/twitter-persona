...
@pytest.fixture
def potus_df():
    """Define a pd.DataFrame containing Tweets from @POTUS."""
...
@pytest.fixture
def RT_FAV_input_df():
    """Define a sample pd.DataFrame containing tweets with FAV and RT."""
    df = pd.DataFrame({
        'User': {1: 'POTUS', 2: 'POTUS', 3: 'POTUS'},
        'text': {
            1: 'RT tweet gets removed',
...
def url_mention_number_input_df():
    """Define a sample pd.DataFrame containing tweets with URL's, mentions and numbers."""
    df = pd.DataFrame({
        'User': {1: 'POTUS', 2: 'POTUS', 3: 'POTUS', 3: 'POTUS'},
        'text': {
            1: 'tweet url https://t.co/q6IH0MDAvk',
...
def stop_words_input_df():
    """Define a sample pd.DataFrame containing tweets with one or multiple stopwords."""
    df = pd.DataFrame({
        'User': {1: 'POTUS', 2: 'POTUS', 3: 'POTUS', 3: 'POTUS'},
        'text': {
            1: 'tweet with stop words',
...
def test_text_clean_col(potus_df):
    """Check for text_clean column in output."""
    clean_df = generalPreprocessing(potus_df)
...
def test_potus_df(potus_df):
    ...
    df_exp = pd.DataFrame({
        ...
        'text_clean': {
            20: 'In America, go forward go together.',
